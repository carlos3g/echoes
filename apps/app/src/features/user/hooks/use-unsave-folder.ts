...
export const useUnsaveFolder = () => {
  const queryClient = useQueryClient();
  const { t } = useTranslation();

  return useMutation<void, HttpError<ApiResponseError>, string>({
    mutationFn: (folderUuid: string) => userService.unsaveFolder(folderUuid),
    onSuccess: (_, folderUuid) => {
      toast.success(t('user.unsaveFolderSuccess'));
    ...
    onError: () => {
      toast.error(t('user.unsaveFolderError'));
    },
  });
};
