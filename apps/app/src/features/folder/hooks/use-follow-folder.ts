...
export const useFollowFolder = (folderUuid: string) => {
  const queryClient = useQueryClient();
  const { t } = useTranslation();

  return useMutation<void, HttpError<ApiResponseError>, void>({
    mutationFn: () => folderService.follow(folderUuid),
    onSuccess: () => {
      toast.success(t('folder.followSuccess'));
      void queryClient.invalidateQueries({ queryKey: queryKeys.folders.detail(folderUuid) });
    },
    onError: () => {
      toast.error(t('folder.followError'));
    },
  });
};
